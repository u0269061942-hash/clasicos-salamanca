...
export default function Motos({ admin, setPagina }) {
  const [motos, setMotos] = useState([])
  const [form, setForm] = useState({ marca: '', modelo: '', anyo: '', cilindrada: '', tipo: '', descripcion: '', propietario: '' })
    ...
    <div style={{ padding: '40px 24px', maxWidth: '1100px', margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
        ...
        {admin && (
          <button onClick={() => setMostrarForm(!mostrarForm)} style={{ fontFamily: 'Georgia, serif', fontSize: '12px', letterSpacing: '2px', padding: '10px 24px', background: '#8B4513', color: '#e8dcc8', border: 'none', cursor: 'pointer' }}>
            {mostrarForm ? 'CANCELAR' : '+ AÑADIR MOTO'}
          </button>
        )}
      </div>
      ...
      {admin && mostrarForm && (
        <div style={{ background: '#120f0a', border: '1px solid #2a2018', borderLeft: '3px solid #8B4513', padding: '24px', marginBottom: '32px' }}>
          <h2 style={{ fontFamily: 'Georgia, serif', fontSize: '18px', color: '#c8a96e', marginBottom: '20px', letterSpacing: '3px' }}>NUEVA MOTO</h2>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
            {[['marca', 'Marca'], ['modelo', 'Modelo'], ['anyo', 'Año'], ['cilindrada', 'Cilindrada'], ['tipo', 'Tipo'], ['propietario', 'Propietario']].map(([key, label]) => (
              <div key={key}>
        ...
        {motos.map(moto => (
          <div key={moto.id} style={{ background: '#120f0a', border: '1px solid #2a2018', overflow: 'hidden' }}>
            ...
            <div style={{ padding: '16px' }}>
              ...
              <div style={{ fontSize: '12px', color: '#8B4513', letterSpacing: '2px', margin: '4px 0' }}>{moto.anyo} {moto.tipo && `· ${moto.tipo}`}</div>
              {moto.cilindrada && <div style={{ fontSize: '12px', color: '#6a5a44' }}>Cilindrada: {moto.cilindrada}</div>}
              {moto.propietario && <div style={{ fontSize: '12px', color: '#6a5a44' }}>Propietario: {moto.propietario}</div>}
              {moto.descripcion && <div style={{ fontSize: '12px', color: '#a89070', marginTop: '8px', lineHeight: '1.5' }}>{moto.descripcion}</div>}
              {moto.historia && <div style={{ fontSize: '12px', color: '#c8a96e', marginTop: '8px', lineHeight: '1.6', fontStyle: 'italic', borderTop: '1px solid #2a2018', paddingTop: '8px' }}>{moto.historia}</div>}
              {admin && (
                <button onClick={() => eliminar(moto.id)} style={{ marginTop: '12px', fontSize: '11px', padding: '4px 12px', background: 'transparent', border: '1px solid #3a2e22', color: '#6a5a44', cursor: 'pointer' }}>
                  Eliminar
                </button>
              )}
            </div>
