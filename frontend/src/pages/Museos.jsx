...
export default function Museos({ admin, setPagina }) {
  ...
  const [tipoFoto, setTipoFoto] = useState('museo')
    ...
    return (
      <div style={{ padding: '40px 24px', maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: '32px' }}>
          ...
          <div style={{ fontSize: '12px', color: '#8B4513', letterSpacing: '2px', margin: '4px 0' }}>{seleccionado.fecha} {seleccionado.lugar && `· ${seleccionado.lugar}`}</div>
          {seleccionado.descripcion && <div style={{ fontSize: '13px', color: '#a89070', marginTop: '8px', lineHeight: '1.6' }}>{seleccionado.descripcion}</div>}
        ...
        {['museo', 'coche', 'moto', 'grupo', 'otro'].map(tipo => {
          const fotosTipo = fotos.filter(f => f.tipo === tipo)
          if (fotosTipo.length === 0) return null
          const labels = { museo: 'MUSEO', coche: 'COCHES', moto: 'MOTOS', grupo: 'GRUPO', otro: 'OTROS' }
          return (
        ...
        {fotos.length === 0 && (
          <div style={{ textAlign: 'center', padding: '40px', color: '#6a5a44', fontFamily: 'Georgia, serif', letterSpacing: '2px' }}>
            AUN NO HAY FOTOS EN ESTE MUSEO
          </div>
        )}

        {admin && (
          <button onClick={() => eliminar(seleccionado.id)} style={{ marginTop: '24px', fontSize: '12px', padding: '8px 20px', background: 'transparent', border: '1px solid #3a2e22', color: '#6a5a44', cursor: 'pointer', fontFamily: 'Georgia, serif', letterSpacing: '2px' }}>
            ELIMINAR MUSEO
          </button>
        )}
  ...
  return (
    <div style={{ padding: '40px 24px', maxWidth: '1100px', margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
        <div>
          <h1 style={{ fontFamily: 'Georgia, serif', fontSize: '36px', letterSpacing: '5px', color: '#c8a96e', fontWeight: 'bold' }}>MUSEOS</h1>
          <p style={{ color: '#6a5a44', letterSpacing: '2px', fontSize: '12px' }}>{museos.length} MUSEOS VISITADOS</p>
        </div>
        {admin && (
          <button onClick={() => setMostrarForm(!mostrarForm)} style={{ fontFamily: 'Georgia, serif', fontSize: '12px', letterSpacing: '2px', padding: '10px 24px', background: '#8B4513', color: '#e8dcc8', border: 'none', cursor: 'pointer' }}>
            {mostrarForm ? 'CANCELAR' : '+ AÑADIR MUSEO'}
          </button>
        )}
      </div>
      ...
      {admin && mostrarForm && (
        <div style={{ background: '#120f0a', border: '1px solid #2a2018', borderLeft: '3px solid #8B4513', padding: '24px', marginBottom: '32px' }}>
          <h2 style={{ fontFamily: 'Georgia, serif', fontSize: '18px', color: '#c8a96e', marginBottom: '20px', letterSpacing: '3px' }}>NUEVO MUSEO</h2>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '16px' }}>
        {museos.map(museo => (
          <div key={museo.id} onClick={() => abrirMuseo(museo)} style={{ background: '#120f0a', border: '1px solid #2a2018', padding: '24px', cursor: 'pointer', borderBottom: '3px solid #8B4513' }}>
            <div style={{ fontFamily: 'Georgia, serif', fontSize: '20px', color: '#c8a96e', fontWeight: 'bold', letterSpacing: '2px', marginBottom: '8px' }}>{museo.nombre}</div>
            <div style={{ fontSize: '12px', color: '#8B4513', letterSpacing: '2px', marginBottom: '4px' }}>{museo.fecha}</div>
            {museo.lugar && <div style={{ fontSize: '12px', color: '#6a5a44' }}>📍 {museo.lugar}</div>}
            {museo.descripcion && <div style={{ fontSize: '12px', color: '#a89070', marginTop: '8px', lineHeight: '1.5' }}>{museo.descripcion}</div>}
